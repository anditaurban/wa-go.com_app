async function fetchAndPopulateOptions(optionId1 = null, optionID2 = null) {
  ...
  try {
    ...
    optionId1Select.innerHTML =
      "<option disabled selected>-Select Pretext-</option>";
    ...
    optionId2Select.innerHTML =
      "<option disabled selected>-Select Tracking ID-</option>";
...
function adminInputHandler(e) {
  ...
  const selectedOption = Array.from(adminDatalist.options).find(
  ...
  if (dataform) {
    dataform.onsubmit = (e) => {
      ...
      if (!csId) {
        Swal.fire(
          "Validation Error",
          "Please select a valid CS from the list",
          "error"
        );
        return false;
      }
